  ...
  return (
    <div className='flex justify-center items-center'>
      <header className={`navbar-container fixed top-0 w-300 z-50 transition-all duration-300 ${scrolled ? 'py-2' : 'py-4'}`}>
        ...
        <div className={`glass-card ${scrolled ? 'shadow-lg' : ''}`}>
          <nav className="flex items-center justify-between px-4 py-3">
            ...
            <ul className="hidden lg:flex items-center space-x-1">
              ...
              <li><Link href="/team" className={`nav-link ${pathname === '/team' ? 'active' : ''}`}>Team</Link></li>
              <li><Link href="/recruitment" className={`nav-link ${pathname === '/recruitment' ? 'active' : ''} px-4!`}>Join Us</Link></li>
        ...
        {/* Mobile Navigation */}
        <div
          className={`glass-card lg:hidden mt-2 overflow-hidden transition-all duration-300 ease-in-out ${navbar ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'
            }`}
        >
          <nav className="py-4 px-6">
            <ul className="space-y-4">
              ...
              <li><Link href="/recruitment" className="block text-sky-400 font-bold py-2">Join Us</Link></li>
